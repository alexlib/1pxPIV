"""
Interrogation module for PIVSuite Python
...
from ..utils.math import create_window_function
from ..utils.image import apply_min_max_filter
...
def interrogate_images(
    ...
    if piv_params.get('ia_preproc_method', 'none').lower() == 'minmax':
        im1 = apply_min_max_filter(
            im1,
            piv_params.get('ia_min_max_size', 15),
            piv_params.get('ia_min_max_level', 0.1)
        )
        im2 = apply_min_max_filter(
            im2,
            piv_params.get('ia_min_max_size', 15),
            piv_params.get('ia_min_max_level', 0.1)
        )
    ...
    if mask1 is not None and not isinstance(mask1, str) and mask1.size > 0:
        im1 = np.where(mask1, np.nan, im1)

    if mask2 is not None and not isinstance(mask2, str) and mask2.size > 0:
    ...
    if ia_method.lower() == 'basic':
    ...
    elif ia_method.lower() == 'defspline':
        ...
        ex_im1, ex_im2, status = process_deform_interrogation(
            im1, im2, ia_n_x, ia_n_y, ia_size_x, ia_size_y,
            ex_im1, ex_im2, status, u0, v0, X, Y, piv_params
        )
...
def process_basic_interrogation(
    ...
    for kx in range(ia_n_x):
        for ky in range(ia_n_y):
            ...
            im_start_x = kx * ia_size_x // 2  # Use half the IA size as step
            im_stop_x = im_start_x + ia_size_x
            im_start_y = ky * ia_size_y // 2  # Use half the IA size as step
            im_stop_y = im_start_y + ia_size_y
...
def process_offset_interrogation(
    ...
    for kx in range(ia_n_x):
        for ky in range(ia_n_y):
            ...
            im1_start_x = kx * piv_params.get('ia_step_x', 16)
            im1_stop_x = kx * piv_params.get('ia_step_x', 16) + ia_size_x
            im1_start_y = ky * piv_params.get('ia_step_y', 16)
            im1_stop_y = ky * piv_params.get('ia_step_y', 16) + ia_size_y
...
def process_deform_interrogation(
    ...
    # Determine which images to deform
    deform_method = piv_params.get('ia_image_to_deform', 'both').lower()

    # Create deformed coordinates
    if deform_method == 'both':
        # Deform both images
        y_deform1 = y_grid + 0.5 * v_interp
        x_deform1 = x_grid + 0.5 * u_interp
        y_deform2 = y_grid - 0.5 * v_interp
        x_deform2 = x_grid - 0.5 * u_interp
    elif deform_method == 'image1':
        # Deform only the first image
        y_deform1 = y_grid + v_interp
        x_deform1 = x_grid + u_interp
        y_deform2 = y_grid
        x_deform2 = x_grid
    elif deform_method == 'image2':
        # Deform only the second image
        y_deform1 = y_grid
        x_deform1 = x_grid
        y_deform2 = y_grid - v_interp
        x_deform2 = x_grid - u_interp
    else:
        raise ValueError(f"Unknown deformation method: {deform_method}")

    # Ensure coordinates are within bounds
    y_deform1 = np.clip(y_deform1, 0, im_size_y - 1)
    x_deform1 = np.clip(x_deform1, 0, im_size_x - 1)
    y_deform2 = np.clip(y_deform2, 0, im_size_y - 1)
    x_deform2 = np.clip(x_deform2, 0, im_size_x - 1)
    ...
    interp_method = piv_params.get('ia_image_interpolation_method', 'spline').lower()
    if interp_method == 'spline':
        interp_method = 'cubic'  # scipy's equivalent to MATLAB's spline

    # Create interpolation functions
    interp_func1 = RegularGridInterpolator(
        (y_coords, x_coords), im1_interp,
        bounds_error=False, fill_value=0,
        method=interp_method
    )
    interp_func2 = RegularGridInterpolator(
        (y_coords, x_coords), im2_interp,
        bounds_error=False, fill_value=0,
        method=interp_method
    )
    ...
    points_to_interp2 = np.column_stack((y_deform2.flatten(), x_deform2.flatten()))
    ...
    for kx in range(ia_n_x):
        for ky in range(ia_n_y):
            ...
            im_start_x = kx * piv_params.get('ia_step_x', 16)
            im_stop_x = kx * piv_params.get('ia_step_x', 16) + ia_size_x
            im_start_y = ky * piv_params.get('ia_step_y', 16)
            im_stop_y = ky * piv_params.get('ia_step_y', 16) + ia_size_y
